 ...
 * System Guide Schema
 *
 * Zod schemas and TypeScript types for the System Guide feature.
 * Guides are user-editable Markdown files with YAML frontmatter.
 *
...
export const GuideHierarchyLevel = ["global", "profile", "project"] as const;
export type GuideHierarchyLevelType = (typeof GuideHierarchyLevel)[number];
 ...
 */
export const GuideSemverRegex = /^\d+\.\d+\.\d+(-[a-zA-Z0-9]+)?$/;
...
export const PreferenceSchema = z.object({
	...
	sourceLevel: z.enum(GuideHierarchyLevel).optional(),
...
export const GuideFrontmatterSchema = z.object({
	...
	/** Semver version */
	version: z.string().regex(GuideSemverRegex).default("1.0.0"),

	/** Hierarchy level (inferred from file location if not specified) */
	level: z.enum(GuideHierarchyLevel).optional(),
...
export const SystemGuideSchema = z.object({
	...
	/** Semver version */
	version: z.string().regex(GuideSemverRegex),

	/** Hierarchy level */
	level: z.enum(GuideHierarchyLevel),
...
/**
 * Get all preferences from a guide (flattened from sections)
 */
export function getPreferencesFromGuide(guide: SystemGuide): Preference[] {
	return guide.sections.flatMap((section) => section.preferences);
}
